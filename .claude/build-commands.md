...
- `task sbom` — Generates CycloneDX 1.6 source-module SBOMs at `sbom.json` and `sbom.xml` via `cyclonedx-gomod mod`, then GPG-signs each output (`sbom.{json,xml}.asc`). Listed in `.releaserc.json`'s GitHub assets and run by `publishCmd`, so each release ships both flavours plus their signatures.
