...
import (
	...
	"github.com/servercurio/go-library-starter/pkg/env"
)

// Config represents the logging configuration for a library consumer.
type Config struct {
	Default *LoggerConfig `yaml:"default" json:"default"`
}

// FromEnv hydrates Default from environment variables prefixed
// "<prefix>_LOG_*".
func (c *Config) FromEnv(prefix string) {
	c.Default.FromEnv(env.AddPrefix(prefix, "log"))
}
...
// DefaultLoggingConfig returns a Config with the default logger enabled at
// info level with pretty-printing on.
func DefaultLoggingConfig() *Config {
	return &Config{
		Default: NewLoggerConfig(zerolog.LevelInfoValue, true, false, true),
	}
}
