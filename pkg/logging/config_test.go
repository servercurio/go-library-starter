...
func TestDefaultLoggingConfig(t *testing.T) {
	...
	assert.NotNil(c)
	assert.NotNil(c.Default)
	assert.True(c.Default.Enabled)
	assert.Equal(zerolog.LevelInfoValue, c.Default.Level)
	assert.True(c.Default.PrettyPrint)
	assert.False(c.Default.IncludeCaller)
}
...
func TestNewConfigFromEnv_AppliesEnvOverrides(t *testing.T) {
	...
	t.Setenv("APP_LOG_PRETTY_PRINT", "false")
	t.Setenv("APP_LOG_INCLUDE_CALLER", "true")

	c := NewConfigFromEnv("APP")
	assert.Equal("debug", c.Default.Level)
	assert.False(c.Default.PrettyPrint)
	assert.True(c.Default.IncludeCaller)
}
...
func TestLoggerConfig_MarshalZerologObject_Smoke(t *testing.T) {
	cfg := DefaultLoggingConfig().Default
...
func TestInitialize_DoesNotPanic(t *testing.T) {
	...
	disabled := &Config{Default: &LoggerConfig{Enabled: false}}
	asrt.NotPanics(t, func() {
		Initialize(disabled)
	})
}
